...
public class MutatorTest {
    ...
    public void setUp() {
        ...
        this.mutatorConfigManager = new MutatorConfigManager(configFilePathOrClasspath);
    }
    ...
    public void testJson() {
        ...
        MutatorConfigInterface mutatorConfig = mutatorConfigManager.getConfig("Json");
        ...
        String result = JMJson.getInstance().toJsonString(resultMap);
        System.out.println(result);
        System.out.println(JMJson.getInstance().toJsonString(mutatorConfig));
        Assert.assertEquals(
    ...
    public void testDelimiter() {
        ...
        MutatorConfigInterface mutatorConfig = new DelimiterMutatorConfig("delimiterTest", ",", "\"",
                new FieldConfigBuilder().setRawData(true).setIgnore(List.of("전화번호", "단지 명")).setCombinedFields(null)
                        .setFormulaFields(new FormulaFieldConfig[]{
                                new FormulaFieldConfig(new String[]{"총 관리비 합계", "세대 수"}, "세대별 평균 관리비", null,
                                        "총 관리비 합계 / 세대 수", null)}).setDataType(null).setDateFormat(null).setFilter(null)
                        .createFieldConfig(), "자치구 명", "동 명", "단지 명", "전화번호", "단지 주소", "연 면적", "관리비 부과 면적", "세대 수",
                "동 수", "년", "월", "총 공용 관리 비용", "총 세대 사용 비용", "총 장기 구선 충당금", "미터제곱 당 공용 관리 비용", "미터제곱 당 세대 사용 비용",
    ...
    public void testFormatted() {
        String testString = "myhost_example_com.cpu-2.cpu-idle 98.6103 1329168255";
        MutatorConfigInterface mutatorConfig = new FormattedMutatorConfig("formattedMutatorTest", true,
                "%hostname" + "\\.%plugin-%pluginInstance\\.\\S+-%typeInstance %value %timestamp",
                Map.of("%hostname", "hostname", "%plugin", "plugin", "%pluginInstance", "pluginInstance",
                        "%typeInstance", "typeInstance", "%value",
                        "value", "%timestamp", "timestamp"),
                new FieldConfigBuilder().setDataType(Map.of("value", DataType.NUMBER, "timestamp", DataType.LONG))
                        .createFieldConfig());
